...
## 5) Curve D — Exponential

$$
f_D(u)=\frac{1-e^{-ku}}{1-e^{-k}},\quad k>0
$$

$$
T_D(t)=T_0+\Delta T\frac{1-e^{-k t/\tau}}{1-e^{-k}}
$$

$$
\dot T_D(t)=\frac{\Delta T}{\tau}\frac{k e^{-k t/\tau}}{1-e^{-k}}
$$

Properties:
- fast initially, then asymptotic slowdown;
- shape controlled by `k` (larger `k` = more front-loaded warming).
...
$$
Then cap by a maximum allowed bag-organ delta.
